...
class AdvCGI(object):
    ...
    <h3>What lauguages can you program in?
    ...
    langSet = ('Python', 'Ruby', 'Java', 'C++', 'PHP', 'C', 'JavaScipt')
    ...
    def showForm(self): 
        ...
        for eachLang in AdvCGI.langSet:
            langStr.append(AdvCGI.langItem % (eachLang, ' checked' if
            eachLang in self.langs else '', eachLang))
    ...
    def doResults(self):
        ...
        print('%s%s' % (AdvCGI.header, AdvCGI.reshtml % (cookStatus, 
        self.who, langList, filename, filedata, AdvCGI.url)))
